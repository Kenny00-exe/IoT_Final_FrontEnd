        ...
        <el-col :span="6">
          ...
          <div class="main-right">
            <chart-card
              ...
              :customClass="`chart-item-bottom-sep`"
            >
              <basic-trend-chart-global
              :data="basicIncrTrendDataGlobal"
                ref="confirmedCountTrendChartGlobal"
            ...
            <chart-card
              ...
              :customClass="`chart-item-bottom-sep`"
            >
              <basic-trend-chart-world
                :data="basicIncrTrendDataWorld"
                ref="confirmedCountTrendChartWorld"
                style="width: 100%; height: 320px"
              />
            </chart-card>
          </div>
      ...
      >
        ...
        <div class="area-data-table-wrapper">
          <el-table
            ...
            style="width: 100%"
          >
            <el-table-column prop="countryName" align="center" label="国家">
            </el-table-column>
            <el-table-column
              prop="confirmedCount"
              align="center"
...
import DataMapWorld from "../components/DataMapWorld";
...
const initCountryConfirmedCountBoardConfig = (resultList = []) => {
  return {
    header: ["国家", "累计确诊", "累计治愈", "累计死亡"],
    headerHeight: 30,
    data: resultList,
    align: ["center"],
    rowNum: 10,
...
export default {
  ...
  data() {
    return {
      ...
      provinceConfirmedCountBoardConfig: initCountryConfirmedCountBoardConfig(),
      provinceDataList: [],
      trendDataList: [],
      confirmedCountList: [],
      basicIncrTrendDataWorld: {
        dateList: [],
        importedIncrDataList: [],
        currentConfirmedIncrDataList: [],
      },
      basicIncrTrendDataGlobal: {
        dateList: [],
        importedIncrDataList: [],
        currentConfirmedIncrDataList: [],
      },
  ...
  methods: {
    queryBasicData() {
      ...
      this.axios
        ...
        .then((res) => {
          ...
          self.basicData.confirmedIncr = res.data[0].sum - res.data[1].sum;
          self.basicData.curedIncr = res.data[0].cured - res.data[1].cured;
          self.basicData.deadIncr = res.data[0].dead - res.data[1].dead;

          self.basicData.confirmedCount = res.data[0].sum;
    ...
    queryCountryDataList() {
      let self = this;
      this.axios.get("http://localhost:8080/api/getData").then((res) => {
        ...
        // 设置各国累计确诊轮播表格数据
        self.setCountryComfirmedCountBoardData(res.data);
        // 设置地图数据
    ...
    queryTrendDataList() {
      ...
      covid19Service.getDailyList().then((res) => {
        ...
        // 重置图表数据
        self.setbasicIncrTrendDataWorld(res.data);
        self.setbasicIncrTrendDataGlobal(res.data);
      });
    },
    setProvinceRankingData(areaList) {
      ...
      let dataValueList = [];
      for (let i = 0; i < 10; i++) {
        provinceList.push(areaList[i].countryName);
    ...
    setbasicIncrTrendDataWorld(data) {
      let dateList = [];
    ...
    setbasicIncrTrendDataGlobal(data) {
      let dateList = [];
      let currentConfirmedIncrList = [];
      let importedIncrList = [];
    ...
    setCountryComfirmedCountBoardData(areaList) {
      ...
      this.provinceConfirmedCountBoardConfig =
        initCountryConfirmedCountBoardConfig(resultList);
    },
    setAreaChartData(areaList) {
      ...
      areaList.forEach((item) => {
        provinceList.push(item.countryName);
    ...
    setBasicData(data) {
      ...
      this.rate = {
        ...
        deadRate: data.deadCount / sum,
      };
    },
    startQueryData() {
      this.queryBasicData();
      this.queryCountryDataList();
      this.queryTrendDataList();
    },
    initAllChart() {
      this.$refs.dataMapWorld.initChart();

        ...
        <el-col :span="6">
          ...
          <div class="main-right">
            ...
            <chart-card
              ...
              :customClass="`chart-item-bottom-sep`"
            >
              <basic-trend-chart-province
                :data="basicIncrTrendDataProvince"
                ref="confirmedCountTrendChartProvince"
                style="width: 100%; height: 320px"
              />
            </chart-card>
          </div>
      ...
      >
        ...
        <div class="area-data-table-wrapper">
          ...
          >
            ...
            <el-table-column
              prop="provinceConfirmedCount"
              align="center"
              label="累计确诊"
            >
            </el-table-column>
            <!-- <el-table-column
              prop="provinceCurrentConfirmedCount"
              align="center"
              label="现有确诊"
            >
            </el-table-column> -->
            <el-table-column
              prop="provinceCuredCount"
              align="center"
              label="累计治愈"
            >
            </el-table-column>
            <el-table-column
              prop="provinceDeadCount"
              align="center"
              label="累计死亡"
            >
            </el-table-column>
  ...
  methods: {
    ...
    queryTrendDataList() {
      ...
      covid19Service.getDailyList().then((res) => {
        ...
        self.setbasicIncrTrendDataProvince(res.data);
      });
    },
    setProvinceRankingData(areaList) {
      ...
      let dataValueList = [];
      for (let i = 0; i < 10; i++) {
        provinceList.push(areaList[i].provinceName);
        dataValueList.push(areaList[i].provinceConfirmedCount);
      }
      let data = {
    ...
    setbasicIncrTrendDataProvince(data) {
      let dateList = [];
      let currentConfirmedIncrList = [];
      let importedIncrList = [];
      ...
      this.basicIncrTrendDataProvince = {
        dateList: dateList,
        importedIncrDataList: importedIncrList,
        currentConfirmedIncrDataList: currentConfirmedIncrList,
        noInFectDataList: noInFectDataList,
    ...
    setProvinceComfirmedCountBoardData(areaList) {
      let resultList = areaList.map((item) => {
        return [
          item.provinceName,
          item.provinceConfirmedCount,
          item.provinceCuredCount,
          item.provinceDeadCount,
        ];
      });
      // 重新生成，否则无法刷新状态
    ...
    setAreaChartData(areaList) {
      ...
      areaList.forEach((item) => {
        ...
        confirmedCountList.push(item.provinceConfirmedCount);
        curedCountList.push(item.provinceCuredCount);
        deadCountList.push(item.provinceDeadCount);
      });
      this.areaData = {
        provinceList: provinceList,

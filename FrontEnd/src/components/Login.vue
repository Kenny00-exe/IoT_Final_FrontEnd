...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      var status = await axios.post('http://localhost:8080/api/v1/LoginService/Login?UserID=' + this.form.id + '&Password=' + this.form.passwd)
      console.log(status)
      if(status.data == -1) {
        return this.$message({
          type: 'error',
...
.login{
  background-image: url("https://www.w3xue.com/files/a201910/20191015084253436.png");
